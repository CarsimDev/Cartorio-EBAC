 ...
 #include <string.h>//Biblioteca responsãvel para strings
 int registro()//Função responsável por cadastrar usuários no sistema
 {
    ...
    strcpy(arquivo, cpf);//Responsável por copiar valores de strings
    
    FILE *file;//Criação de arquivos
 ...
 int consulta()
 {
 	...
 	if(file == NULL)
 	{
 	   	printf("Este ID não foi localizado.\n");
 		system("pause");
	}
 ...
 int deletar()//Ínicio da exclusão de usuário
 {
    char cpf[40];
    char cpf2[200];
 ...
 int main()
 {
	...
	for(laco=1;laco=1;)
	{
	    ...
	    printf("Opção:"); //Fim do menu
 	    ...
 	    scanf("%d",&opcao); //Armazenando info. do usuário
